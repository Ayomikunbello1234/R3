  ...
  let activeForm = document.querySelector(".card-form.organization") || forms[0];
  let currentStep = 0;

  function render(stepIndex) {
    currentStep = stepIndex;
    const steps = activeForm.querySelectorAll(".form");

    // 1. Update form step visibility & classes
    steps.forEach((step, idx) => {
      step.classList.toggle("active", idx === currentStep);
      step.classList.toggle("prev", idx < currentStep);
    });

    // 2. Update progress bars
    bars.forEach((bar, idx) => {
      bar.style.display = idx < steps.length ? "block" : "none";
      bar.classList.toggle("active", idx <= currentStep);
    });
  }

  function switchForm(targetClass) {
    const nextForm = document.querySelector(`.card-form.${targetClass}`);
    if (!nextForm) return;
  ...
  document.addEventListener("click", (e) => {
    ...
    e.preventDefault();
    const totalSteps = activeForm.querySelectorAll(".form").length;

    if (nextBtn && currentStep < totalSteps - 1) {
      render(currentStep + 1);
    } else if (backBtn && currentStep > 0) {
      render(currentStep - 1);
    }
  });

  // Initial load
  if (activeForm) render(0);
});

const industry = document.getElementById("industry");
const otherindustry = document.getElementById("otherindustry");
const communitytype = document.getElementById("community-type");
const othercommunity = document.getElementById("othercommunity");
...
communitytype.addEventListener("change", function(){
    if(communitytype.value === "other"){
        othercommunity.style.display = "block";
    }
    else{
...
const OrganizationState = document.getElementById("organizationstate");
const OrganizationCity = document.getElementById("organizationcity");
 ...
 const locations = {
    Abia: [
        ...
        "Obingwa", "Ohafia", "Osisioma Ngwa", "Ugwunagbo",
    ...
    Bauchi: [
        "Alkaleri", "Bauchi", "Bogoro", "Darazo", "Dass", "Gamawa",
        "Ganjuwa", "Giade", "Itas/Gadau", "Jama'are", "Katagum",
        "Kirfi", "Misau", "Ningi", "Shira", "Tafawa Balewa",
        "Toro", "Warji", "Zaki", "Damban"
    ],
    ...
    Nasarawa: [
        ...
        "Kokona", "Lafia", "Nasarawa", "Nasarawa Eggon",
        "Obi", "Toto", "Wamba"
    ],
    ...
    Oyo: [
        "Akinyele", "Atiba", "Atisbo", "Egbeda", "Ibadan North",
        ...
        "Iwajowa", "Kajola", "Lagelu", "Ogbomoso North",
        "Ogbomoso South", "Ogo Oluwa", "Olorunsogo", "Oluyole",
        "Ona Ara", "Orelope", "Oriire", "Oyo East", "Oyo West",
    ...
    Yobe: [
        ...
        "Nangere", "Nguru", "Potiskum", "Tarmuwa", "Yunusari",
        "Yusufari"
    ],

    Zamfara: [
        "Anka", "Bakura", "Birnin Magaji/Kiyaw", "Bukun Yum",
...
addState(CommunityState);
addState(OrganizationState);

function addCity(stateSelect, citySelect){
...
addCity(CommunityState, CommunityCity);
addCity(OrganizationState, OrganizationCity);